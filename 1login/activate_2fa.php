<?php
	...
	$Email = $_SESSION['email'];  // Almacena el email de usuario de la sesión
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$QrCodeImage = $Tfa->getQRCodeImageAsDataUri($Email, $Secret);  // Genera un código QR como URI de datos

		// Intenta actualizar el secreto de 2FA en la base de datos y muestra el resultado
		if (UpdateUser2FASecret($Email, $Secret)) {
			echo '<div class="info">2FA activado con éxito.<br>GUÁRDALO BIEN, POR EL AMOR DE DIOS<br><br><img src="' . htmlspecialchars($QrCodeImage) . '" alt="Código QR para autenticación 2FA"><br>Escanee el código QR con su aplicación de autenticación. (Google Authenticator)</div>';
		} else {
	...
	function UpdateUser2FASecret($Email, $Secret): bool {
		...
		$Query      = $Connection->prepare("UPDATE pps_users SET usu_verification_code = ? WHERE usu_email = ?");  // Prepara la consulta SQL
